...
function setLineView(){
  var textarea = document.getElementById("my-textarea");
  //var count = countLines(textarea);
  var lines = textarea.value.split('\n');
  var left = "";
  //var offsetWidth = document.getElementById("my-textarea").offsetWidth
  for(var i = 0;i < lines.length;i++){
    if (lines[i] === "") {
      left += `<span class="darkerLine">${(i+1)}</span><br>`
      /*var count = lineAmount(lines[i], offsetWidth);
      console.log(i +":"+ count);
...
function lineAmount(line, textwidth) {
  var width = displayTextWidth(line, document.getElementById("my-textarea").style.fontFamily);
  var count = Math.ceil(width / textwidth);
  console.log(count);
  return count;
}
